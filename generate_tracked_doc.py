...
def add_tracked_p(doc, runs_spec, align=WD_ALIGN_PARAGRAPH.LEFT, double_spaced=True, rev_id_start=1, author="Nolan Yu", date_str="2026-08-08T12:00:00Z"):
    """
...
def build_tracked_manuscript():
    ...
    orig_abstract = (
        ...
        "Using a discovery dataset from peripheral blood samples (GSE55763, n = 2,711) and a clinical validation cohort from tumor tissue (TCGA-SKCM, n = 395), "
        "we performed a two-tailed Welch’s t-test and quality filtering to identify differentially methylated positions (DMPs) located in gene promoter regions (TSS200 and TSS1500). "
        "Kaplan-Meier survival analysis and log-rank testing identified 24 significant CpG sites (p < 0.05) strongly associated with overall survival. "
        "Interestingly, the cohort exhibited a mixed epigenetic directionality: 15 sites showed protective hypermethylation (associated with longer survival, representing silenced oncogenes like ZCCHC11 and KMO), "
        "and 9 sites showed hazardous hypermethylation (associated with shorter survival, representing silenced tumor suppressors like RGS21, CYMP, and AMPD2). "
        "Furthermore, to evaluate the diagnostic utility of these methylated CpG sites under realistic biological tissue heterogeneity, we implemented a machine learning framework incorporating attribute selection (Gain Ratio) "
        ...
        "The top classifiers achieved up to 92.15% 10-fold cross-validation accuracy and an Area Under the Receiver Operating Characteristic curve (AUC-ROC) of 0.9723. "
        ...
        "These findings provide a robust data-driven foundation for a multi-site epigenetic prognostic panel and outline immediately actionable, pathway-level therapeutic strategies for melanoma."
    )
    ...
    p1 = (
        "Cutaneous melanoma, arising from the malignant transformation of melanocytes, represents the most lethal form of skin cancer. "
        "Although it accounts for less than 5% of all dermatological cancers, it is responsible for over 75% of skin-cancer-related deaths [5]. "
        "The high mortality rate is primarily driven by its genomic instability, rapid metastatic spread, and resistance to traditional chemotherapy. "
        "Over the past decade, the clinical landscape of melanoma has been revolutionized by targeted therapies (such as BRAF and MEK inhibitors) "
        "and immune checkpoint inhibitors (such as anti-PD-1 and anti-CTLA-4 antibodies). However, a significant proportion of patients do not respond "
        "to these therapies or eventually develop resistance, highlighting the critical need for novel diagnostic biomarkers and prognostic classifiers to stratify patient risk and guide personalized treatment [5]."
    )
    rev_id = add_tracked_p(doc, [('plain', p1)], rev_id_start=rev_id)
    
    p2 = (
        "While somatic mutations (e.g., BRAF, NRAS, and NF1) have traditionally dominated molecular research in melanoma, "
        "epigenetic alterations have recently emerged as critical drivers of tumorigenesis [4]. Epigenetics refers to heritable changes in gene expression "
        "that do not alter the primary DNA sequence. Among these, DNA methylation—the covalent addition of a methyl group (-CH3) to the 5-carbon of a cytosine ring in a CpG dinucleotide—is the most widely studied."
    )
    rev_id = add_tracked_p(doc, [('plain', p2)], rev_id_start=rev_id)
    
    p3 = (
        "In normal cells, CpG islands (regions with a high frequency of CpG sites) located in gene promoters are generally unmethylated, allowing active transcription. "
        "In cancer cells, this landscape is severely disrupted, characterized by global hypomethylation (which promotes genomic instability and reactivates retrotransposons) "
        "and localized hypermethylation of CpG islands within tumor-suppressor gene promoters, which physically blocks transcription factor binding and recruits methyl-CpG-binding domain (MBD) proteins to compact chromatin, silencing gene expression."
    )
    rev_id = add_tracked_p(doc, [('plain', p3)], rev_id_start=rev_id)
    
    p4 = (
        "This project aims to bridge the gap between epigenetic discovery and clinical application. By leveraging large-scale public datasets, "
        "we sought to systematically identify CpG promoter sites that are differentially methylated in melanoma and directly correlate with patient survival. "
        "Understanding the precise methylation status of these loci not only provides a powerful prognostic toolkit but also reveals key biological pathways that can be targeted therapeutically to arrest tumor progression [7]."
    )
    ...
    p5_part1 = (
        "DNA methylation is catalyzed by a family of enzymes known as DNA methyltransferases (DNMTs). DNMT3A and DNMT3B act as de novo methyltransferases, "
        "establishing new methylation marks on unmethylated DNA, while DNMT1 acts as a maintenance methyltransferase, copying methylation patterns to the newly synthesized strand during DNA replication. "
        "The removal of methyl groups, or demethylation, occurs either passively through the absence of maintenance during replication or actively via the Ten-Eleven Translocation (TET) family of dioxygenases (TET1, TET2, TET3), "
        "which oxidize 5-methylcytosine (5mC) to 5-hydroxymethylcytosine (5hmC), leading to eventual reversion to cytosine [4]."
    )
    rev_id = add_tracked_p(doc, [('plain', p5_part1)], rev_id_start=rev_id)
    
    # Page 5 yellow highlight rewrite tracking
    p5_part2_orig_prefix = "In oncogenesis, promoter hypermethylation is a well-established mechanism for silencing tumor-suppressor genes, acting as one of the \"two hits\" in Knudson's hypothesis for gene inactivation. Conversely, promoter hypomethylation can lead to the aberrant "
    p5_part2_del_word = "activation"
    p5_part2_ins_word = "transcription and pathological activation"
    p5_part2_orig_suffix = " of oncogenes or cancer-testis antigens. Usually measurement of these methylation levels is quantified using \"beta values\" (β-values), which range from 0 (completely unmethylated) to 1 (fully methylated), representing the proportion of methylated alleles at a given locus across a cell population [6]."
    
    rev_id = add_tracked_p(doc, [
        ('plain', p5_part2_orig_prefix),
        ('del', p5_part2_del_word),
        ('ins', p5_part2_ins_word),
        ('plain', p5_part2_orig_suffix)
    ], rev_id_start=rev_id)
    ...
    p_lim1 = (
        "The clinical approval of global epigenetic drugs, such as the DNMT inhibitors (DNMTis) azacitidine and decitabine, represents a milestone in cancer therapy, particularly in hematological malignancies. "
        "These nucleoside analogs incorporate into DNA during replication and covalently trap DNMTs, leading to active degradation of the enzymes and global, genome-wide hypomethylation."
    )
    p_lim2 = (
        "However, these broad-spectrum drugs have demonstrated very limited success in solid tumors, including melanoma. The primary biological limitation is their lack of specificity. "
        "A global demethylating agent cannot distinguish between a methylated tumor suppressor (where reactivation is therapeutically beneficial) and a methylated oncogene or retrotransposon (where reactivation is highly detrimental). "
        "As shown in this study, the melanoma methylome contains both hypermethylated tumor suppressors and hypomethylated oncogenes. "
        "Administering a systemic, non-specific DNMT inhibitor risks reactivating oncogenes (such as ZCCHC11, KMO, and EPHA10) and inducing genomic instability, potentially accelerating tumor aggressiveness. "
        "Thus, there is an urgent need for precision locus-specific editing or pathway-targeted alternative therapies."
    )
    rev_id = add_tracked_p(doc, [('plain', p_lim1)], rev_id_start=rev_id)
    rev_id = add_tracked_p(doc, [('plain', p_lim2)], rev_id_start=rev_id)
    
    # STATEMENT OF PROBLEM (Page 8 - Yellow Highlight Rewrite Tracking)
    add_h(doc, "STATEMENT OF PROBLEM", level=1)
    
    orig_problem = "In the current article we try to elucidate if DNA methylation profiles can be utilized to develop a robust, statistically significant diagnostic and prognostic panel for cutaneous melanoma , and how the identified epigenetic alterations can be therapeutically targeted."
    new_problem = "Can promoter DNA methylation signatures be leveraged to construct a robust, statistically validated diagnostic and prognostic panel for cutaneous melanoma, and how can the identified epigenetic alterations be therapeutically targeted using translationally viable, non-CRISPR pharmacological interventions in clinical practice?"
    ...
    p_curation = (
        "We analyzed publicly available genomic and clinical datasets to identify and validate diagnostic and prognostic epigenetic signatures in cutaneous melanoma. "
        "For the platform annotations, we retrieved coordinates and gene symbol mappings for the 485,577 probes on the Illumina HumanMethylation450 BeadChip Array (platform GPL13534) from the NCBI Gene Expression Omnibus (GEO). "
        "For the initial discovery dataset, we downloaded preprocessed beta values and delta beta scores for 839 candidate CpG sites from the peripheral blood methylation dataset GSE55763 (n = 2,711). "
        "For clinical validation, we obtained raw DNA methylation data (processed via the SeSAMe pipeline) and clinical metadata from the NIH Genomic Data Commons (GDC) Data Portal for the TCGA-SKCM cohort (initial cohort n = 470)."
    )
    rev_id = add_tracked_p(doc, [('plain', p_curation)], rev_id_start=rev_id)
    
    add_h(doc, "Software and Computational Environment", level=2)
    p_env = (
        "The bioinformatic pipeline was developed in Python (version 3.8+). Data loading, manipulation, and clinical aggregation were performed using pandas (version 1.3+) and NumPy (version 1.20+). "
        "Statistical analyses, including the two-tailed Welch's t-test and cumulative survival distributions, were executed via the SciPy library (version 1.7+). "
        "Kaplan-Meier modeling and log-rank test evaluations were carried out using the Lifelines library (version 0.26+), machine learning algorithms were built using scikit-learn (version 1.0+), "
        "and high-resolution figures were generated using Matplotlib and Seaborn."
    )
    ...
    p_screen = (
        "To isolate reliable disease-associated epigenetic marks, we applied a two-tailed Welch’s t-test independently to the 839 candidate CpG sites to identify differentially methylated positions (DMPs) "
        "between normal blood and melanoma samples. The t-statistic was calculated under a conservative assumption of n = 10 per group:\n"
        "t = |ΔBeta| / √((RMSDDiseased² / n) + (RMSDNormal² / n))\n"
        "CpG sites with p ≥ 0.05 were discarded, retaining 758 DMPs. We then applied a strict effect-size quality filter to exclude highly variable sites. "
        "Probes were excluded if the within-group root-mean-square deviation (RMSD) in either the diseased or normal group exceeded half of the absolute effect size (|ΔBeta|):\n"
        "RMSDDiseased ≥ 0.5 × |ΔBeta|  and  RMSDNormal ≥ 0.5 × |ΔBeta|\n"
        "This reliability screen filtered the candidate list to highly consistent marks. Using the GPL13534 annotation file, we restricted these candidate sites to genomic promoter regions, "
        "defined as TSS200 (within 200 bp upstream of the transcription start site) and TSS1500 (200 to 1500 bp upstream) regions. This genomic isolation yielded 214 promoter-associated CpG sites for clinical validation."
    )
    rev_id = add_tracked_p(doc, [('plain', p_screen)], rev_id_start=rev_id)
    
    # Clinical Deduplication and Cohort Matching (Page 10 Yellow Rewrite Tracking)
    ...
    new_dedup = (
        ...
        "We implemented a group-by aggregation by unique case ID (`cases.case_id`), recovering the maximum available follow-up or overall survival duration across recorded `days_to_death` (for deceased patients) "
        "and `days_to_last_follow_up` (for censored patients), while retaining primary vital status annotations."
    )
    dedup_suffix = (
    ...
    p_ml_train = (
        "To evaluate whether the identified promoter CpG sites can serve as an accurate diagnostic machine learning classifier for melanoma detection, "
        "we formulated a binary classification task that compares melanoma tumor tissue methylation profiles with normal tissue profiles across the 24 significant CpG descriptors. "
        "Input features consisted of quantitative promoter CpG methylation beta values (β-values ranging from 0.0 to 1.0).\n"
        "Attribute selection was conducted using Mutual Information and Gain Ratio evaluation (`GainRatioAttributeEval`) to quantify the informativeness of each CpG descriptor relative to the disease class label. "
        "Gain Ratio scores were computed to rank probe discriminative power while adjusting for multi-valued feature bias.\n"
        "Six distinct machine learning classification algorithms were implemented and trained:\n"
        "1. SPegasos / Support Vector Machine (SVM): Radial Basis Function (RBF) kernel SVM utilizing stochastic gradient descent optimization to establish a margin-based hyperplane separating diseased from normal tissue.\n"
        ...
        "4. Multi-Layer Perceptron (MLP): Feed-forward deep neural network architecture with two hidden layers (16 and 8 neurons) utilizing ReLU activations and Adam optimization.\n"
    ...
    rev_id = add_tracked_p(doc, [('plain', p_ml_train)], rev_id_start=rev_id)
    
    # Independent External Dataset Validation (New Subsection Insertion Tracking)
    ...
    rev_id = add_tracked_p(doc, [('ins', ind_val_text)], rev_id_start=rev_id)
    
    # Survival Analysis
    add_h(doc, "Survival Analysis and Target Curation", level=2)
    p_surv = (
        "For each of the 214 promoter CpG sites, the 395 matched patients were stratified into high-expression and low-expression groups at the median beta value. "
        "To prevent statistical artifacts in sparsely populated groups, we excluded sites containing fewer than 5 patients in either group. "
        "We modeled cumulative overall survival (OS) distributions using the Kaplan-Meier estimator and performed independent log-rank tests to identify loci significantly associated with patient survival (p < 0.05). "
        "Finally, we performed pathway mapping to classify the significant genes as oncogenes or tumor suppressors and map their interactions to identify targetable downstream pathway nodes."
    )
    rev_id = add_tracked_p(doc, [('plain', p_surv)], rev_id_start=rev_id)
    ...
    proc_steps = [
        "1. Platform Annotation Retrieval: Download the platform annotation file GPL13534 containing genomic coordinates and gene symbol mappings for the 485,577 probes on the Illumina HumanMethylation450 array.",
        "2. Discovery Dataset Retrieval: Download preprocessed methylation beta values and delta beta scores for 839 candidate CpG sites from GSE55763.",
        ...
        "6. TCGA Clinical Data Retrieval: Download the clinical manifest and `clinical.tsv` file containing follow-up and vital status details for 470 patients in the TCGA SKCM cohort.",
    ...
    # Step 13 (Tracked Insertion)
    proc_step13 = "13. Independent External Validation Testing: Evaluate frozen trained models on an un-seen independent external dataset (GSE105191, n = 40) to compute external accuracy, sensitivity, specificity, and ROC-AUC curves."
    rev_id = add_tracked_p(doc, [('ins', proc_step13)], rev_id_start=rev_id)
    
    proc_step14 = "14. Therapeutic Target Mapping: Classify significant genes as oncogenes or tumor suppressors via literature review. Map these genes to downstream pathways and detail alternative pharmacological targeting strategies."
    rev_id = add_tracked_p(doc, [('plain', proc_step14)], rev_id_start=rev_id)
    ...
    # Results text above Table 5 (Yellow Rewrite Tracking)
    orig_results_text = "learning performance metrics. Write much more text explaining all these results. Anyway – these are your main results of the paper."
    new_results_text = (
    ...
    rev_id = add_tracked_p(doc, [
        ('plain', "To convert the identified CpG methylation signatures into a clinical diagnostic system, we evaluated six machine learning classifiers using quantitative promoter CpG methylation beta values (β-values).\n"
                  "Attribute selection via Gain Ratio (`GainRatioAttributeEval`) identified the top 10 most influential CpG descriptors discriminating melanoma from normal tissue. Table 4 presents the top 10 strongest positive descriptors ranked by Gain Ratio score.\n\n"),
        ('del', orig_results_text),
        ('ins', new_results_text)
    ], rev_id_start=rev_id)
    
    # Independent External Dataset Validation (Results Subsection Insertion Tracking)
    ...
    rev_id = add_tracked_p(doc, [('ins', res_ind_val_text)], rev_id_start=rev_id)
    
    # DATA ANALYSIS & RECOMMENDATIONS (UNTOUCHED)
    ...
    p_ubiad1 = (
        "UBIAD1 (UbiA Prenyltransferase Domain Containing 1) is a well-documented tumor suppressor that synthesizes Coenzyme Q10 and vitamin K2, "
        "inducing oxidative-stress-mediated apoptosis in cancer cells. Under the classical model, silencing UBIAD1 should lead to worse patient outcomes. "
        "However, the survival data showed a paradox: patients with higher UBIAD1 promoter methylation lived significantly longer (8.92 vs. 5.49 years). "
        "This paradoxical protective effect may reflect a tissue-specific metabolic wiring in melanoma, where the accumulation of prenyl substrates or altered intracellular cholesterol pathways (which UBIAD1 regulates) alters the sensitivity of melanocytes to oxidative stress, highlighting the complexity of epigenetic signaling."
    )
    rev_id = add_tracked_p(doc, [('plain', p_ubiad1)], rev_id_start=rev_id)
    
    add_h(doc, "CONCLUSION", level=1)
    p_conc = (
        "This study successfully established a bioinformatic pipeline that identifies diagnostic and prognostic DNA methylation signatures in cutaneous melanoma. "
        "By addressing a critical patient record deduplication error in the TCGA-SKCM dataset, the cohort was expanded from 42 to 395 patients, providing the statistical power necessary to detect a highly significant 24-CpG survival panel.\n"
        "The findings support the hypothesis that the melanoma epigenetic landscape exhibits mixed directionality, characterized by the hypermethylation and silencing of key tumor suppressors (RGS21, AMPD2, LPPR4) alongside the hypomethylation and activation of strong oncogenes (ZCCHC11, KMO, EPHA10). "
        "These results demonstrate that DNA methylation profiles can serve as reliable prognostic classifiers, and underscore the necessity of moving away from global, non-specific epigenetic therapies in favor of targeted, pathway-specific therapeutic interventions."
    )
    ...
    p_rec = (
        "While locus-specific epigenome editing using dead Cas9 fusions (dCas9-DNMT3A to silence oncogenes and dCas9-TET1 to reactivate tumor suppressors) is highly promising in preclinical settings, "
        "it is not yet approved for clinical use due to delivery, safety, and immunogenicity barriers [8]. To bypass these limitations, we propose directly targeting the downstream pathways or employing clinically approved pharmacological agents that counteract the identified epigenetic alterations (Figure 4)."
    )
    rev_id = add_tracked_p(doc, [('plain', p_rec)], rev_id_start=rev_id)
    
    # Save tracked document
    out_docx_path = r"C:\Users\ethan\.gemini\antigravity\scratch\melanoma_methylation_paper\manuscript_tracked.docx"
    doc.save(out_docx_path)
    print(f"Tracked DOCX successfully saved to {out_docx_path}!")

if __name__ == '__main__':
    build_tracked_manuscript()
