...
def add_tracked_p(doc, runs_spec, align=WD_ALIGN_PARAGRAPH.LEFT, double_spaced=True, rev_id_start=1, author="Nolan Yu", date_str="2026-08-08T15:44:00Z"):
    """
...
def add_h(doc, text, level=2, tracked_type=None, author="Nolan Yu", date_str="2026-08-08T15:44:00Z", rev_id=100):
    p = doc.add_paragraph()
    p_format = p.paragraph_format
    p_format.line_spacing = 1.15
    ...
    if tracked_type == 'ins':
        rPr_xml = f'<w:rPr><w:rFonts w:ascii="Times New Roman" w:hAnsi="Times New Roman"/><w:sz w:val="{font_size*2}"/>'
        if bold: rPr_xml += '<w:b/>'
        if italic: rPr_xml += '<w:i/>'
        rPr_xml += '</w:rPr>'
        escaped = text.replace('&', '&amp;').replace('<', '&lt;').replace('>', '&gt;')
        ins_xml = (
            f'<w:ins {nsdecls("w")} w:id="{rev_id}" w:author="{author}" w:date="{date_str}">'
            f'<w:r>{rPr_xml}<w:t xml:space="preserve">{escaped}</w:t></w:r>'
            f'</w:ins>'
        )
        p._p.append(parse_xml(ins_xml))
    elif tracked_type == 'del':
        rPr_xml = f'<w:rPr><w:rFonts w:ascii="Times New Roman" w:hAnsi="Times New Roman"/><w:sz w:val="{font_size*2}"/>'
        if bold: rPr_xml += '<w:b/>'
        if italic: rPr_xml += '<w:i/>'
        rPr_xml += '</w:rPr>'
        ...
        del_xml = (
            f'<w:del {nsdecls("w")} w:id="{rev_id}" w:author="{author}" w:date="{date_str}">'
            f'<w:r>{rPr_xml}<w:delText xml:space="preserve">{escaped}</w:delText></w:r>'
            f'</w:del>'
        )
        p._p.append(parse_xml(del_xml))
    else:
...
def build_tracked_doc():
    ...
    p5_part2_prefix = "In oncogenesis, promoter hypermethylation is a well-established mechanism for silencing tumor-suppressor genes, acting as one of the \"two hits\" in Knudson's hypothesis for gene inactivation. Conversely, "
    p5_part2_orig_yellow = "promoter hypomethylation can lead to the aberrant activation of oncogenes or cancer-testis antigens."
    p5_part2_new_yellow = "promoter hypomethylation can lead to the aberrant transcription and pathological activation of oncogenes or cancer-testis antigens."
    ...
    rev_id = add_tracked_p(doc, [
        ...
        ('del', p5_part2_orig_yellow),
        ('ins', p5_part2_new_yellow),
        ('plain', " "),
        ('del', "Usually"),
        ('ins', "The"),
    ...
    add_h(doc, "STATEMENT OF PROBLEM", level=1, tracked_type=None)
    ...
    # Clinical Deduplication (Yellow Rewrite Tracking)
    ...
    # Machine Learning Model Training (Screenshot 3 & 4 Tracked Edits)
    ...
    add_h(doc, ind_val_title, level=2, tracked_type='ins', rev_id=250)
    rev_id = add_tracked_p(doc, [('ins', ind_val_text)], rev_id_start=rev_id)
    
    # Survival Analysis (Screenshot 5 Tracked Edit)
    ...
    add_h(doc, res_ind_val_title, level=2, tracked_type='ins', rev_id=300)
    ...
    doc.save(out_path)
    print(f"Tracked DOCX successfully saved to {out_path}!")
